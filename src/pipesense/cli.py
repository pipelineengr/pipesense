...
def parse_args(argv=None):
    ...
    p_status = sub.add_parser(
        "status", help="Show data source configuration for a site"
    )
...
async def _run_single_site(
    ...
    try:
        with (
            ArchiveWriter(db_path, run_id=run_id, site_id=site.id) as archive,
            AlarmLog(db_path, run_id=run_id, site_id=site.id) as alarm_log,
        ):
            while not stop_event.is_set():
                if (
                    args.duration is not None
                    and (_time.monotonic() - start) >= args.duration
                ):
                    break
                ...
                for reading in readings:
                    ...
                    for event in engine.process(reading):
                        ...
                        print(
                            f"  [{site.id}] [{event.severity.value}] "
    ...
    tasks = [
        asyncio.create_task(
            _run_single_site(
                ...
                engine=DetectionEngine(
                    site
                ),  # one engine per site — state is independent
...
def _status(args, config_path: str) -> None:
    ...
    for site in sites:
        ...
        for ch in site.channels:
            print(
                f"    {ch.id:10s} {ch.type:12s} [{ch.unit}] poll={ch.poll_interval_s}s"
            )
...
def _sites(config, site_args: list[str] | None) -> list:
    ...
    for sid in site_args:
        ...
        if site is None:
            print(
                f"Site {sid!r} not found in config. "
                f"Available: {[s.id for s in config.sites]}",
                file=sys.stderr,
            )
            sys.exit(1)
